...
def test_moving_event_invalidates_linked_entities(db_service, temporal_manager):
    ...
    # 2. Create an event
    event = Event(name="Departs Rivendell", lore_date=1000.0, type="journey")
    ...
    state_at_1500_after = temporal_manager.get_entity_state_at(entity.id, time=1500.0)
...
def test_adding_relation_invalidates_target_entity(db_service, temporal_manager):
    ...
    event = Event(name="Coronation", lore_date=3019.0, type="milestone")
    db_service.insert_event(event)

    # 2. Warm cache (no relations yet)
    state_before = temporal_manager.get_entity_state_at(entity.id, time=3020.0)
    ...
    temporal_manager.on_relation_changed("new_rel_id", event.id, entity.id)
...
def test_deleting_relation_invalidates_target_entity(db_service, temporal_manager):
    ...
    event = Event(name="Arrives", lore_date=1000.0, type="arrival")
    db_service.insert_event(event)
    ...
    # Get the relation ID
    relations = db_service.get_incoming_relations(entity.id)
    ...
    temporal_manager.on_relation_changed(rel_id, event.id, entity.id)
