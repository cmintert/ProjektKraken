...
def test_mainwindow_check_unsaved_changes(qtbot):
    ...
    with patch("src.app.main.DatabaseWorker"), \
        ...
        with patch("PySide6.QtWidgets.QMessageBox.warning", return_value=QMessageBox.Save) as mock_msg:
        ...
        with patch("PySide6.QtWidgets.QMessageBox.warning", return_value=QMessageBox.Discard) as mock_msg:
             with patch.object(mock_event_editor, "_on_save") as mock_on_save:
                assert window.check_unsaved_changes(window.event_editor)
                mock_on_save.assert_not_called()
            
        # 4. Dirty Editor, User selects Cancel
        with patch("PySide6.QtWidgets.QMessageBox.warning", return_value=QMessageBox.Cancel) as mock_msg:
             with patch.object(mock_event_editor, "_on_save") as mock_on_save:
