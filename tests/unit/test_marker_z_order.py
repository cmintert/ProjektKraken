...
def _setup_view_with_pixmap(qtbot, width: int = 200, height: int = 200) -> MapGraphicsView:
...
class TestMarkerZOrder:
    def test_trajectory_z_order_inherits_from_marker(self, map_graphics_view):
        """Test that trajectory keyframes sit just underneath their marker."""
        ...
        # 5. Verify Z-values
        # Trajectory path should be at custom_z - 0.3
        assert view._trajectory.trajectory_path_item.zValue() == custom_z - 0.3
        
        # Dots should be at custom_z - 0.2
        for dot in view._trajectory.keyframe_items:
            assert dot.zValue() == custom_z - 0.2
            
        # Labels should be at custom_z - 0.1
        for label in view._trajectory.keyframe_label_items:
            assert label.zValue() == custom_z - 0.1
            
    def test_trajectory_update_z_values(self, map_graphics_view):
        """Test that update_z_values correctly resyncs Z-values."""
        view = map_graphics_view
        ...
        # Verify sync
        assert view._trajectory.trajectory_path_item.zValue() == new_z - 0.3
        assert view._trajectory.keyframe_items[0].zValue() == new_z - 0.2
