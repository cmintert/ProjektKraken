    ...
    with tempfile.NamedTemporaryFile(suffix=".png", delete=False) as tmp:
        ...
        try:
            with pytest.raises(Exception):
                asset_store.import_image("event", "event-123", tmp.name)
...
def test_delete_files_moves_to_trash(asset_store, test_image):
    ...
    # Import image first
    image_rel, thumb_rel, _ = asset_store.import_image(
        "event", "event-123", test_image
    )
...
def test_restore_files_success(asset_store, test_image):
    ...
    asset_store.restore_files(
        trash_img_rel, image_rel, trash_thumb_rel, thumb_rel
    )
