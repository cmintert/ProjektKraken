...
def test_get_resource_path_bundled():
    ...
    with patch.object(sys, "_MEIPASS", "/bundled/app"):
        result = get_resource_path("test/resource.txt")
        
        assert result == os.path.join("/bundled/app", "test/resource.txt")
...
def test_get_user_data_path_windows():
    """Test get_user_data_path on Windows."""
    with patch("sys.platform", "win32"):
        with patch.dict(os.environ, {"APPDATA": "C:\\Users\\Test\\AppData\\Roaming"}):
            ...
            if sys.platform == "win32":  # Only check on actual Windows
                assert "AppData" in result or "ProjektKraken" in result
...
def test_ensure_worlds_directory_creates_directory():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        with patch("src.core.paths.get_worlds_dir", return_value=Path(tmpdir) / "worlds"):
